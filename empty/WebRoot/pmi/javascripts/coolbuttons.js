...
function doUp(){
	...
	var cDisabled=el.cDisabled;
	cDisabled=(cDisabled!=null);
	if((el.className=="coolButton")&&!cDisabled){makeRaised(el);
	}
}
...
function disable(el){
	...
	var cDisabled=el.cDisabled;
	cDisabled=(cDisabled!=null);
	if(!cDisabled){el.cDisabled=true;
		if(document.getElementsByTagName){el.innerHTML="<span style='background: buttonshadow; filter: chroma(color=red) dropshadow(color=buttonhighlight, offx=1, offy=1); height: 100%;'>"+"<span style='filter: mask(color=red); height: 100%;'>"+el.innerHTML+"</span>"+"</span>";
	}else{
...
function enable(el){
	var cDisabled=el.cDisabled;
	cDisabled=(cDisabled!=null);
	if(cDisabled){el.cDisabled=null;
...
function addToggle(el){
	...
	if(!cToggle&&!cDisabled){
		el.cToggle=true;if(el.value==null)
		el.value=0;
	if(el.onclick!=null)
...
function removeToggle(el){
		...
		makeFlat(el);
		if(el.cToggle_onclick!=null){el.onclick=el.cToggle_onclick;el.cToggle_onclick=null;
		}
	}
}
...
function makeRaised(el){
	with(el.style){borderLeft="1px solid buttonhighlight";
...
function makeGray(el,b){
	...
	for(var i=0;i<imgs.length;i++){imgs[i].style.filter=filtval;
	}
}
